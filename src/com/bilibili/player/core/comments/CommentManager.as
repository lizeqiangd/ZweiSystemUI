...
//	import org.lala.comments.CommentSpaceManager;
...
//	import org.lala.utils.GeneralFactory;
	...
	public class CommentManager
	{
		...
		/** 同一模式下的同屏弹幕计数:用于只限制某类弹幕的同屏密度 **/
		protected var liveTotal:Number = 0;
		
		/** 同一个时间区间平凡弹幕的预备存储点 **/
		protected static var prepare_stack:Vector.<IComment> = new Vector.<IComment>();
		
		/** 弹出准备好的区间弹幕:重新按时间,日期排序 **/
		public static function popComments(clip:DisplayObjectContainer, isPlaying:Boolean):void
		{
			//trace("CommentManager.popComments")
			/** 按照时间进行排列:解决了同一区间下滚动弹幕必然在固定弹幕上的问题 **/
			if (prepare_stack.length)
			{
				prepare_stack.sort(stimeCmp);
				if (isPlaying)
				{
					for each (var cmt:IComment in prepare_stack)
					{
						clip.addChild(cmt as DisplayObject);
						cmt.start();
					}
				}
				else
				{
					for each (var cmt2:IComment in prepare_stack)
					{
						clip.addChild(cmt2 as DisplayObject);
						cmt2.start();
						/** 暂停时发送的弹幕,在显示后立即暂停 **/
						cmt2.pause();
					}
				...
				/** 清除所有的弹幕 **/
				prepare_stack = new Vector.<IComment>();
			}
		}
		
		/** 弹幕排序 **/
		protected static function stimeCmp(a:IComment, b:IComment):int
		{
			var sa:Number = a.data.stime;
			var sb:Number = b.data.stime;
			if (sa > sb)
				return 1;
			else if (sa < sb)
				return -1;
			...
			{
				var dta:String = a.data.date;
				var dtb:String = b.data.date;
		...
		public function insert(data:Object):void
		{
			...
			{
				/** 普通弹幕进行脚本代码呈现尝试 **/
				if (parseInt(obj.mode) <= 5)
				{
					var ret:Boolean = config.commentTriggerManager.trigger(obj);
					if (!ret)
					{
						this.start(obj);
					}
			...
			{
				/** 单设置preview只是不插入队列,要开始播放还是要border属性,或者live **/ /** 可以启用过滤系统与同屏密度的设置 **/
				if (validate(obj))
				{
					this.start(obj);
				}
			...
			else if (p == this.pointer)
			{
				if (obj.stime < oldPosition)
				{
					this.pointer++;
				}
			}
		...
		protected function start(data:Object):void
		{
			...
			/** 压入准备栈,在所有弹幕准备完成后一同(排序)出栈 **/ /** 如果是预览,就不入栈 **/
			if (!data['preview'])
			{
				/** 如果是用户发 送的弹幕,将提前显示 **/
				if (!data['border'])
				{
					CommentManager.prepare_stack.push(cmt);
				}
				else
				{
					clip.addChild(cmt as DisplayObject);
					cmt.start();
					/** 暂停延迟,以便让滚动弹幕显示,并不修改其位置 **/
					if (ValueObjectManager.getMediaProvider.isPause)
					{
		...
		{
			var cmt:Comment = commentFactory.getObject() as Comment;
