	...
	public class sb_hexagon_core
	{
		...
		*/
		public function sb_hexagon_core($target:*, $maskTarget:*, $scrollBar:Sprite, $scrollLine:Sprite, $tween:Number = 0, $elastic:Boolean = true, $lineAbleClick:Boolean = false, $mouseWheel:Boolean = true, $direction:String = "L",$textMode=false)
		{
			if (! $textMode)
			{
				...
				textMode = false;
				_target = DisplayObjectContainer($target);
		...
		*/
		...
		private function checkAbled():void
		{
			...
			{
				if (_target["height"] >= _target["textHeight"])
				{
					_scrollBar.visible = false;
					_scrollLine.visible = false;
					if (_downBtn)
		...
		private function scrollBarLength():void
		{
			...
			if (textMode)
			{
				_scrollBar[_length] = _scrollLine[_length] * _target["height"] / _target["textHeight"];
			}
			else
			{
		...
		private function timeHandler(e:TimerEvent):void
		{
			if (textMode)
			{
				_targetPoint =((_scrollBar[_coor] - _scrollBarOriginalPoint[_coor]) / (_scrollLine[_length] - _scrollBar[_length]));
				_target["scrollV"] =1+ Math.floor(1+_target["numLines"]- (_target["numLines"]*_target[_length]/_target["textHeight"]))*_targetPoint
				/*
				trace("********************");
				trace("test",Math.round(2+_target["numLines"]- (_target["numLines"]*_target[_length]/_target["textHeight"])));
				trace("test",_targetPoint);
				trace("scrollV",(_target["textHeight"]/_targetPoint -1)*20+1)
				trace("_targetPoint",_targetPoint);
				trace("_target[textHeight]",_target["textHeight"]);
				trace("_scrollBar[_coor]",_scrollBar[_coor]);
				trace("_scrollBarOriginalPoint[_coor]",_scrollBarOriginalPoint[_coor]);
				trace("_target[_length]",_target[_length]);
				trace("_scrollLine[_length]",_scrollLine[_length]);
				trace("_scrollBar[_length]", _scrollBar[_length]);
				trace("_target[numLines]", _target["numLines"]);
				trace("_target[scrollV]", _target["scrollV"]);
				trace("********************");
				*/
			}
			else
		...
		//注册点移到左上角
		private function setRegistration($target:*):void
		{
